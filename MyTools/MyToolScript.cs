...
public class MyToolScript : MonoBehaviour
{
    ...
    public Vector3 ReturnVector3Center(Vector3 inputVector3, Grid inputGrid)
    {
    ...
    public bool GameObjectWithinDistance(float inputDistance, GameObject inputGameObject, Vector2 inputCheckedPosition) // Determines if the mouse hovered gameobject's collider's closest point is within distance of the player
    {
    ...
    //returns Vector3Int of whatever is input
    public Vector3Int CalculateGridPosition(Vector3 inputVector, Grid inputGrid)
    {
    ...
    public RaycastHit2D PositionRayCast(Vector3 inputPosition, LayerMask checkedLayer) //raycast from mouse position to screen checking for whatever layers are passed in
    {
    ...
    //offset is for determining the center of the gameObject 
    ...
    /// </summary>
    public bool Vector3WithinDistance(float distance, bool is2D, Vector3 checkedPosition, Vector3 playerPosRounded) //Checks if the input Vector3 is within distance of the player
    {
        float difference;
        distance *= distance; 
        ...
        //Debug.Log("The difference in distance is " + difference);
